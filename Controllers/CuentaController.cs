    ...
    try
    {
        using (SqlConnection con = new(_contexto.Conexion))
        {
            using (SqlCommand cmd = new("sp_validar_usuario", con))
            {
                ...
                while (dr.Read())
                {
                    string userId = dr["idUsuario"].ToString();
                    string nombreUsuario = dr["nombreUsuario"].ToString();
                    string contrasena = dr["contrasena"].ToString();
                    ...
                    if (!string.IsNullOrEmpty(nombreUsuario))
                    {
                        List<Claim> claims = new List<Claim>()
                        {
                            ...
                            // Añadimos un claim para identificar si el usuario tiene permiso de crear curso
                            new Claim("CrearCursoPermission", 
                                (nombreUsuario == "Nicole" && u.contrasena == "1234") ? "true" : "false")
                        };
